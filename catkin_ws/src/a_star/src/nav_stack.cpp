...
void scan_neighbours (vector<vector<int> > maze,
...
int pathfinder (coords_t * start, coords_t * end, vector<vector<int> > maze) {
    open[0] = start;
...
void start_end_scan (coords_t *start, coords_t *end, vector<vector<int> > maze) {    
    // Dummy values 
...
int find_heading (coords_t * start, vector<vector<int> > maze) {
    for (int i = -1; i < 2; i++) {
        for (int d = -1; d < 2; d++) {
...
void generate_path (vector<vector<int> > maze) {    
